...
#let title_slide(title, title_size, author1, author2, color1: color1, color2: color2, n: 6) = {
  ...
  place(horizon+center, rect(fill:white, stroke: 5pt, width: 80%, height: 80%)[#title])
...
#title_slide([Генерация синтетических данных для обучения моделей машинного обучения], 42pt, [Григорьев Семён], [АС-24-05])

#slide([Постановка задачи], [Пусть имеется обучающая выборка, которая может быть недостаточно большой или неравномерной.], [Генерация синтетических данных может помочь в увеличении объема данных и улучшении качества модели.])

#slide([Методы генерации синтетических данных], [1. Генеративные модели (например, GANs, VAEs). 2. Аугментация данных (например, повороты, масштабирование, шум). 3. Симуляция (например, физические модели, агентные модели).], [Выбор метода зависит от типа данных и задачи машинного обучения.], title_size: 40pt)

#slide([Преимущества генерации синтетических данных], [1. Увеличение объема данных. 
2. Улучшение качества модели. 
3. Сохранение конфиденциальности данных.], [Генерация синтетических данных может быть особенно полезной в областях, где сбор реальных данных затруднен или дорогостоящий.], title_size: 40pt, text_size: 22pt)

#slide([Заключение], [Генерация синтетических данных является мощным инструментом для улучшения моделей машинного обучения, особенно в условиях ограниченных данных.], [Выбор метода генерации синтетических данных должен основываться на конкретных требованиях задачи и характеристиках данных.], title_size: 40pt, text_size: 22pt, is_last: true)
